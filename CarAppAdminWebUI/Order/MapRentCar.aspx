<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MapRentCar.aspx.cs" Inherits="CarAppAdminWebUI.Order.MapRentCar" %>
        ...
        <dl>
            ...
            <dd>
                <ul>
                    ...
                    <li style="display: none;">共有<span id="neworderinfo">0</span>个订单需要派车</li>
                </ul>
            </dd>
        </dl>
        <dl >
...
<script type="text/javascript">
    ...
    function setMapMarker(orderid) {
        ...
        var data_info = [];
        $.get("/Car/CarLocationHandler.ashx", { "action": "orderlist", orderid: "<%=orderid %>", timestamp: new Date().getTime() }, function (date) {
            if (date.resultcode == 0) {
                $.messager.alert("提示信息", "订单已派或状态异常", function () {
                    window.close();
                });
                return;
            }
            ...
            for (var m = 0; m < date.orders.length; m++) {
                ...
                if (date.orders[m].CarUseWay == "随叫随到") {
                    if (orderid && date.orders[m].orderId == orderid) {
                        $("#queryResult").append("<dd style='color:red'>订单编号：" + date.orders[m].orderId + "<a style='cursor:pointer; color:blue;margin-left:10px;'  onclick='onShowClickOrder(\"" + date.orders[m].orderId + "\")' >[当前订单]</a></dd>");
                    } else {
                        $("#queryResult").append("<dd style='color:red'>订单编号：" + date.orders[m].orderId + "<a style='cursor:pointer; color:blue;margin-left:10px;'  onclick='onShowClickOrder(\"" + date.orders[m].orderId + "\")' >查看</a></dd>");
                    }

                } else {
                    if (orderid && date.orders[m].orderId == orderid) {
                        $("#queryResult").append("<dd>订单编号：" + date.orders[m].orderId + "<a style='cursor:pointer; color:blue;margin-left:10px;'  onclick='onShowClickOrder(\"" + date.orders[m].orderId + "\")' >[当前订单]</a></dd>");
                    } else {
                        $("#queryResult").append("<dd>订单编号：" + date.orders[m].orderId + "<a style='cursor:pointer; color:blue;margin-left:10px;'  onclick='onShowClickOrder(\"" + date.orders[m].orderId + "\")' >查看</a></dd>");
                    }
                }
                $("#queryResult").fadeIn();
            }
            if (flags == 0) {
                $("#queryResult dd:first a").html("[当前订单]");
            }

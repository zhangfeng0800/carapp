...
{
    public partial class ChangePassword : PageBase.PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request.Form["action"] == "SubmitNewPW")
            {
                ...
                if (thisUserAccount.Password.Trim() != Md5Password)
                {
                    Response.Write("{\"Message\":\"原密码不正确！\"}");
                    Response.End();
                }
                thisUserAccount.Password = EncryptAndDecrypt.Encrypt(Request.Form["newPW"]);

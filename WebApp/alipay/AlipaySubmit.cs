...
namespace Com.Alipay
{
    ...
    {
        #region 字段
        private static string GATEWAY_NEW = "https://mapi.alipay.com/gateway.do?_input_charset=utf-8";
        ...
        public void GoToPay(string orderCode, string money, string quantity,string serviceInfo,string style)
        {
            ...
            sParaTemp.Add("notify_url", WebConfigurationManager.AppSettings["paybackUrl"] + "/AliPayBgTransReturn.aspx");
            sParaTemp.Add("return_url", WebConfigurationManager.AppSettings["paybackUrl"] + "/OrderPaySuccess.aspx");
            //业务参数
            sParaTemp.Add("out_trade_no", orderCode);
            sParaTemp.Add("subject",serviceInfo);
            ...
            sParaTemp.Add("body", style);
            Dictionary<string, string> dicPara = new Dictionary<string, string>();
            dicPara = BuildRequestPara(sParaTemp);
            if (dicPara.Count != 0)
            {
                HttpContext.Current.Response.Write("<form name='alipayForm' method='get' action='" + GATEWAY_NEW + "'>");
                foreach (KeyValuePair<string, string> temp in dicPara)
                {

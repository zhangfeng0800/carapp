...
function $V() {
	return $SelectDate(arguments[0]).value;
}
function fPopCalendar(evt, popCtrl, dateCtrl) {
	...
	with($SelectDate("calendardiv").style) {
		left = point.x + "px";
...
function fSetDate(iYear, iMonth, iDay) {
	...
	gdCtrl.value = iYear + splitChar + iMonthNew + splitChar + iDayNew;//这里是时间赋值
	fHideCalendar();
}
function fHideCalendar() {
	$SelectDate("calendardiv").style.visibility = "hidden";
...
function fSetSelected() {
	var iOffset = 0;
	var iYear = parseInt($SelectDate("tbSelYear").value);
	var iMonth = parseInt($SelectDate("tbSelMonth").value);
	var aCell = $SelectDate("cellText" + arguments[0]);
...
function fDrawCal(iYear, iMonth, iCellHeight, iDateTextSize) {
	...
	var styleTD = " valign='middle' align='center' style='height:" + iCellHeight + "px;font-size:" + iDateTextSize + "px;";
	var dateCal = "";
	dateCal += "<tr>";
	for (var i = 0; i < 7; i++) {
...
function fSetYearMon(iYear, iMon) {
	$SelectDate("tbSelMonth").options[iMon - 1].selected = true;
	for (var i = 0; i < $SelectDate("tbSelYear").length; i++) {
		if ($SelectDate("tbSelYear").options[i].value == iYear) {
			$SelectDate("tbSelYear").options[i].selected = true;
		}
	}
	fUpdateCal(iYear, iMon);
}
function fPrevMonth() {
	var iMon = $SelectDate("tbSelMonth").value;
	var iYear = $SelectDate("tbSelYear").value;
	if (--iMon < 1) {
		iMon = 12;
		iYear--;
	}
...
function fGetXY(aTag) {
	...
	do {
		...
		//alert(oTmp.tagName + "-" + oTmp.className + "-" + oTmp.offsetParent.tagName);
	} while (oTmp.tagName.toUpperCase() != 'HTML' && oTmp.tagName.toUpperCase() != 'BODY');
	return pt;
}
function getDateDiv() {
	var noSelectForIE = "";
	...
	dateDiv += "<tr>";
	dateDiv += "<td><input type='button' id='PrevMonth' value='<' style='height:20px;width:20px;' onclick='fPrevMonth()'>";
	...
	dateDiv += "</select></td><td>";
	dateDiv += "<input type='button' id='NextMonth' value='>' style='height:20px;width:20px;' onclick='fNextMonth()'>";
	dateDiv += "</td>";
	dateDiv += "</tr><tr>";
	...
	dateDiv += "<span style='cursor:pointer;font:12px/24px 宋体,Verdana;' onclick='fSetDate(giYear,giMonth,giDay)' onmouseover='this.style.color=\"" + gcMouseOver + "\"' onmouseout='this.style.color=\"#000000\"'>" + strToday + ":" + giYear + strYear + giMonth + strMonth + giDay + strDay + "</span>";
	dateDiv += "</tr></tr>";
	dateDiv += "</table></div>";
	return dateDiv;
}
